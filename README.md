...
| [`taxonomy-selection-with-retry`](./workflows/taxonomy-selection-with-retry) | Merges three free tools (`search_agents`, `search_costs`, `search_reliability`) to rank real candidates by cluster/capability, cost, and success rate, then invokes the top pick -- with a self-healing retry against a real same-cluster fallback if it fails | Yes, both paths -- a real live failure (`translation-v1` genuinely failed with `output_schema_invalid:translation_missing` on a generic task) correctly triggered the honest "no real fallback available" report when none existed in that cluster; after fixing the task text to genuinely suit the selected agent, the full happy path was also confirmed: real completion, real 8p charge, proof verified true |
